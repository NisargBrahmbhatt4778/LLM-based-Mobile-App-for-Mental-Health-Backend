...
from tools.event_handler import ALL_HANDLERS, clear_all_event_handlers
...
tool_kwargs = {
    ...
    "memory_model": "llama3-8b-8192",
    "memory_embedding_type": "llama",
            ...
            try:
                ...
                async def respond(self, messages: List[Dict[str, str]], *args: Any, **kwargs: Any, ) -> Any:
                    ...
                    response = await openai.ChatCompletion.acreate(
                        model=self.assistant_settings["model"], #this is gpt3.5 or gpt 4 (or llama3)
                ...
                if i == 2:
                    logger.error(e)
                    await websocket.send_text("0-Sorry, the service that I rely on from Groq is currently down. Please try again.")
...
def update_model(user_id):
    ...
    if model_name != userAssistants[0].assistant_settings["model"]:
        ...
        ALL_HANDLERS["OnModelChanged"](**kwargs) #only used by image creation tool
...
def recommend_based_on_feedback(last_session_feedback):                            #retourne 2 liste d'exercices (enjoyment et practice) à faire selon les notes attribuées par l'utilisateur, ne propose que des exo déjà rencontrés donc
...
def prepare_llm_recommendation_prompt(next_exercises, feedback_recommendations, exercise_descriptions_short):
    ...
    if next_exercises:
        ...
        prompt += ", ".join([f"{ex} ({exercise_descriptions_short[ex]})" for ex in next_exercises]) + ". "          #create a list of strings, where each string is an exercise name + its short description in parentheses, the join() method is then used to concatenate the strings into a single string, with each string separated by ", ".
...
async def initialize_or_fetch_exercise_tracking(db, user_id):                #récupère le suivi des exercices
    ...
    if not doc.exists:
        ...
        user_exercise_tracking_ref.set(exercise_tracking_default)             #gpt conseille d'ajouter await au début de la ligne pour profiter pleinement de l'asynchronisme
        return exercise_tracking_default
    else:
        return doc.to_dict()
    
#la sortie est un dict par défaut ou contenant les données du suivi des exercices
...
"""
    ...
    print("full prompt diplayable : line ~1488")
    #print(full_prompt)
    return full_prompt
...
def check_qa(user_query, embeddings, questions, answers, user_assistants):              #check if there is already a close question and an answer
