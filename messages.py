...
class Messages:
    ...
    async def initilize_messages(self):
        ...
        self.messages = self.get_messages_from_firestore()                 #récupère les messages sur la db
        print("success : messages récupérés depuis firestore")

        while self.total_char_count() > self.max_char_count:               #tant qu'il y a trop de caractères, on supprime le premier message pour libérer de la place
            self.messages.pop(1)
