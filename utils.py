...
import nltk
import string
from groq import Groq  #for llama3
...
def query_llama(query):
    ...
    response = client.chat.completions.create(
        #model="llama3-70b-8192",
        model="llama3-8b-8192", #use that one if too many tokens used
...
def process_user_query(user_query, db, model, corpus_embeddings, data_dict):
    ...
    user_exact_problem=query_llama(user_query)
...
def load_qna_from_firestore(db):
    """
    Load QnA data from Firestore.

    Args:
        db: Firestore database reference.

    Returns:
        A tuple containing the loaded questions, embeddings tensor, and answers.
    ...
    if doc.exists:
        data = doc.to_dict()
        embeddings_tensor = torch.tensor(json.loads(data['vectorized_questions']))             #conversion en tenseur pytorch
